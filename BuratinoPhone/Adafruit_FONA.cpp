...
// Reading SIM Sender phone book's 
boolean Adafruit_FONA::getSIMSender(uint8_t i, char *sender, int senderlen) {
  // Ensure text mode and all text mode parameters are sent.
//  if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
//  if (! sendCheckReply(F("AT+CSDH=1"), ok_reply)) return false;


  DEBUG_PRINT(F("AT+CPBR="));
  DEBUG_PRINTLN(i);
  ...
  // Parse the second field in the response.
  boolean result = parseReplyQuoted(F("+CPBR: "), sender, senderlen, ',', 1);
  // Drop any remaining data from the response.
  flushInput();
  return result;
}

// Reading SIM Sendername phone book's 
boolean Adafruit_FONA::getSIMSendernName(uint8_t i, char *sendername, int senderlen) {
  // Ensure text mode and all text mode parameters are sent.
  ...
  // Parse the second field in the response.
  boolean result = parseReplyQuoted(F("+CPBR: "), sendername, senderlen, ',', 3);
  // Drop any remaining data from the response.
  flushInput();
  return result;
}

boolean Adafruit_FONA::getBatt(char *buff, uint16_t maxlen) {
  getReply(F("AT+CBC"), (uint16_t) 10000);
  if (strncmp(replybuffer, "+CBC: ", 6) != 0)
    return false;

  char *p = replybuffer+6;
  uint16_t lentocopy = min(maxlen-1, (int)strlen(p));
  strncpy(buff, p, lentocopy+1);
  buff[lentocopy] = 0;
